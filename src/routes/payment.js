     ...
     const {firstName, lastName, emailId} = req.user;
        try {
         ...
         if (!membershipAmounts[membershipTypes]) {
            return res.status(400).json({
                success: false,
                message: `Invalid membership type: ${membershipTypes}. Available types: ${Object.keys(membershipAmounts).join(', ')}`
            });
        }
        ...
        //     }
        // });
        } catch (error){ console.error("Payment creation error:", error);
